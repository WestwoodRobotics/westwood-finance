...
export function createTeamView(getTeam: () => string) {
  const teamOrders = $derived(dataService.orders.filter(o => matchesTeam(o, getTeam())));
  ...
  const teamFunds = $derived(
    getTeam() === 'Westwood Overall'
      ...
      : dataService.funds.filter(f => {
          ...
          return r === s || r.includes(s) || r === 'all' || r === 'westwood overall';
        })
  );
  ...
  const totalRaised = $derived(teamFunds.reduce((sum, f) => sum + (Number(f.Amount) || 0), 0));
  const totalSpent = $derived(financialOrders.reduce((sum, o) => sum + (o.total || 0), 0));
  ...
  return {
    get teamOrders() { return teamOrders; },
    get teamFunds() { return teamFunds; },
    get financialOrders() { return financialOrders; },
    get totalRaised() { return totalRaised; },
    get totalSpent() { return totalSpent; },
    get netBalance() { return netBalance; },
    get monthlyTrends() { return monthlyTrends; },
  };
}
