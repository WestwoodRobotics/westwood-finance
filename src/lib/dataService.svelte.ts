...
class DataStore {
  ...
  normalizeOrders(data: unknown[]): Order[] {
    ...
    return rows
      ...
      .map((o, index) => {
        const stableInput = encodeURIComponent(
          String(o["Item"] ?? o["item"] ?? "item"),
        );
        let baseId = String(
          o["UUID"] ||
            o["Order UUID"] ||
        ...
        return {
          ...
          category: String(o["Category"] || o["category"] || "miscellaneous")
            .toLowerCase()
            .trim(),
          team: String(o["Team"] ?? o["team"] ?? o["user"] ?? ""),
          timestamp: String(o["Timestamp"] ?? o["timestamp"] ?? ""),
          total:
            Number(o["Total"] ?? o["total"]) ||
            Number(o["Price"] ?? o["price"]) *
              Number(o["Quantity"] ?? o["quantity"]) ||
            0,
          status,
  ...
  normalizeFunds(data: unknown[]): Fund[] {
    ...
    return (data as Record<string, unknown>[]).map((f, index) => {
      const stableInput = encodeURIComponent(
        String(f["Type"] ?? f["Source"] ?? "fund"),
      );
      let baseId = String(
        f["id"] || f["UUID"] || `fund-${index}-${stableInput}`,
      );
      let finalId = baseId;
      let counter = 1;
      while (seenIds.has(finalId)) {
  ...
  async load(force = false, silent = false): Promise<void> {
    if (
      ...
      this.lastFetched &&
      Date.now() - this.lastFetched < 120000
    ) {
      return;
    }
    ...
    try {
      ...
      this.persist();
      console.log(
        `DataStore: ${this.orders.length} orders, ${this.funds.length} funding entries`,
      );
    } catch (e) {
